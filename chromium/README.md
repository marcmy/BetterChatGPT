...
- **Native hang recorder** — low-overhead diagnostics for the severe ChatGPT/tool-call UI freezes that can occur even with BetterChatGPT disabled. On Chromium/Edge, a service worker can record when the page stops heartbeating. Diagnostic data contains timing/count metadata, not prompt or tool-result text.
...
You can also clone/download the repository and load the `chromium` directory directly.
...
BetterChatGPT runs locally in the ChatGPT page. The native hang recorder records performance/timing metadata and coarse UI counts only; it does not intentionally capture prompts, tool results, filenames, tokens, cookies, or conversation text. See [PRIVACY.md](PRIVACY.md).
